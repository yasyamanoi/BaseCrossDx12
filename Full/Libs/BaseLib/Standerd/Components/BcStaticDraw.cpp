...
namespace basecross {
	...
	void BcStaticDraw::OnUpdateConstantBuffers() {
		...
		auto stage = std::dynamic_pointer_cast<Stage>(scene->GetActiveStage());
		...
		if (stage) {
			myCamera = std::dynamic_pointer_cast<PerspecCamera>(stage->GetCamera());
			myLightSet = stage->GetLightSet();
		}
		else {
			return;
		}
		...
		if (gameObject) {
			...
			//シーンのコンスタントバッファ
			{
				...
				auto view = (XMMATRIX)((bsm::Mat4x4)myCamera->GetViewMatrix());
				auto proj = (XMMATRIX)((bsm::Mat4x4)myCamera->GetProjMatrix());
				auto worldView = world * view;
				m_constantBuffer.worldViewProj = bsm::Mat4x4(XMMatrixTranspose(XMMatrixMultiply(worldView, proj)));
				//フォグの設定
				if (m_fogEnabled)
				{
					...
					if (start == end)
					{
						...
						m_constantBuffer.fogVector = bsm::Vec4(fullyFogged);
					}
					else
					{
						...
						m_constantBuffer.fogVector = bsm::Vec4((worldViewZ + wOffset) / (start - end));
					}
					m_constantBuffer.fogColor = (bsm::Col4)m_fogColor;
				}
				else
				{
					m_constantBuffer.fogVector = bsm::Vec4(g_XMZero);
					m_constantBuffer.fogColor = bsm::Vec4(g_XMZero);
				}
				//ライトの決定
				for (int i = 0; i < myLightSet->GetNumLights(); i++) {
					m_constantBuffer.lightDirection[i] = (bsm::Vec4)myLightSet->GetLight(i).m_directional;
					m_constantBuffer.lightDiffuseColor[i] = (bsm::Vec4)myLightSet->GetLight(i).m_diffuseColor;
					m_constantBuffer.lightSpecularColor[i] = (bsm::Vec4)myLightSet->GetLight(i).m_specularColor;
				}
				//ワールド行列
				m_constantBuffer.world = bsm::Mat4x4(world);
				m_constantBuffer.world.transpose();

				XMMATRIX worldInverse = XMMatrixInverse(nullptr, world);
				m_constantBuffer.worldInverseTranspose[0] = bsm::Vec4(worldInverse.r[0]);
				m_constantBuffer.worldInverseTranspose[1] = bsm::Vec4(worldInverse.r[1]);
				m_constantBuffer.worldInverseTranspose[2] = bsm::Vec4(worldInverse.r[2]);

				XMMATRIX viewInverse = XMMatrixInverse(nullptr, view);
				m_constantBuffer.eyePosition = bsm::Vec4(viewInverse.r[3]);

				bsm::Col4 diffuse = bsm::Col4(1.0f);
				bsm::Col4 alphaVector = (bsm::Col4)XMVectorReplicate(1.0f);
				bsm::Col4 emissiveColor = bsm::Col4(0.0f);
				bsm::Col4 ambientLightColor = (bsm::Col4)myLightSet->GetAmbient();

				// emissive と ambientとライトをマージする
				m_constantBuffer.emissiveColor = (emissiveColor + (ambientLightColor * diffuse)) * alphaVector;
				m_constantBuffer.specularColorAndPower = bsm::Col4(0, 0, 0, 1);

				// xyz = diffuse * alpha, w = alpha.
				m_constantBuffer.diffuseColor = bsm::Col4(XMVectorSelect(alphaVector, diffuse * alphaVector, g_XMSelect1110));

				auto mainLight = myLightSet->GetMainBaseLight();
				bsm::Vec3 calcLightDir = bsm::Vec3(mainLight.m_directional) * bsm::Vec3(-1.0f);

				bsm::Vec3 lightAt(myCamera->GetAt());

				bsm::Vec3 lightEye(calcLightDir);

				lightEye *= bsm::Vec3(Shadowmap::GetLightHeight());
				lightEye += lightAt;

				bsm::Vec4 LightEye4 = bsm::Vec4(lightEye, 1.0f);
				m_constantBuffer.lightPos = LightEye4;
				bsm::Vec4 eyePos4 = bsm::Vec4((bsm::Vec3)myCamera->GetEye(), 1.0f);
