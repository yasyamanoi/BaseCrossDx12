	...
	void SpPNTStaticDraw::OnUpdateConstantBuffers() {
		...
		if (gameObject) {
			...
			//シーンのコンスタントバッファ
			{
				...
				world.transpose();
				//ビューと射影行列を得る
				bsm::Mat4x4 viewMat = myCamera->GetViewMatrix();
				//転置する
				viewMat.transpose();
				bsm::Mat4x4 projMat = myCamera->GetProjMatrix();
				//転置する
				projMat.transpose();
				...
				if (IsOwnShadowActive()) {
					...
					m_simpleConstant.lightView = bsm::bsmUtil::transpose(lightView);
					m_simpleConstant.lightProjection = bsm::bsmUtil::transpose(lightProj);
				}
			}
		}
	}
