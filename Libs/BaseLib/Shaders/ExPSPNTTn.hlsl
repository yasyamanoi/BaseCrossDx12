...
float4 CalcUnshadowedAmountPCF2x2(int lightIndex, float4 vPosWorld)
{
	...
	vShadowDepths.x = (float)shadowMap.Sample(sampleClamp, vShadowTexCoord);
	vShadowDepths.y = (float)shadowMap.Sample(sampleClamp, vShadowTexCoord + float2(vTexelUnits.x, 0.0f));
	vShadowDepths.z = (float)shadowMap.Sample(sampleClamp, vShadowTexCoord + float2(0.0f, vTexelUnits.y));
	vShadowDepths.w = (float)shadowMap.Sample(sampleClamp, vShadowTexCoord + vTexelUnits);
