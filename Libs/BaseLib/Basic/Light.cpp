...
namespace basecross {
	...
	void LightSet::OnCreate() {
		//デフォルトのライトを作成する
		const XMFLOAT3 defaultDirections[3] =
		{
		...
		const XMFLOAT4 defaultDiffuse[3] =
		{
		...
		const XMFLOAT4 defaultSpecular[3] =
		{
			{ 0.3231373f, 0.3607844f, 0.3937255f,0.0f },
			{ 0.0000000f, 0.0000000f, 0.0000000f,0.0f },
			{ 1.0000000f, 0.9607844f, 0.8078432f,0.0f },
		};
		const XMFLOAT4 defaultAmbient = { 0.05333332f, 0.09882354f, 0.1819608f ,0.0f };
		m_lights.resize(3);
		for (UINT i = 0; i < 3; i++) {
