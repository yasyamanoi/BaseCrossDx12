...
namespace basecross {
	...
	BaseUI::BaseUI(UINT frameCount, ID3D12Device* pDevice, ID3D12CommandQueue* pCommandQueue) :
		m_width(0.0f),
		m_height(0.0f)
	{
	...
	void BaseUI::Initialize(ID3D12Device* pDevice, ID3D12CommandQueue* pCommandQueue)
	{
	...
	void BaseUI::UpdateLabels(const std::wstring& uiText)
	{
		// Update the UI elements.
		m_textBlocks[0] = { uiText, D2D1::RectF(0.0f, 0.0f, m_width, m_height), m_textFormat.Get() };
	}

	void BaseUI::Render(UINT frameIndex)
	{
	...
	void BaseUI::Resize(ComPtr<ID3D12Resource>* ppRenderTargets, UINT width, UINT height)
	{
