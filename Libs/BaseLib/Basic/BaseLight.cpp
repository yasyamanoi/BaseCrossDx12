...
namespace basecross {
	...
	void LightSet::SetLight(size_t index, const BaseLight& light) {
		if (index >= m_lights.size()) {
			throw BaseException(
				L"ライトのインデックスが範囲外です",
				Util::SizeTToWStr(index),
	...
	void LightSet::AddLight(const BaseLight& light) {
	...
	shared_ptr<LightSet>
		...
		try {
			...
			static const Vec4 defaultDiffuse[3] =
			{
			...
			static const Vec4 defaultSpecular[3] =
			{
				{ 0.3231373f, 0.3607844f, 0.3937255f,0.0f },
				{ 0.0000000f, 0.0000000f, 0.0000000f,0.0f },
				{ 1.0000000f, 0.9607844f, 0.8078432f,0.0f },
			};
			static const Vec4 defaultAmbient = { 0.05333332f, 0.09882354f, 0.1819608f ,0.0f };
