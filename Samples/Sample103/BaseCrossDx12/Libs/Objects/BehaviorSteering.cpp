...
namespace basecross {
	...
	///	SeekSteering行動クラス
	//--------------------------------------------------------------------------------------
	SeekSteering::SeekSteering(const std::shared_ptr<GameObject>& GameObjectPtr) :
		SteeringBehavior(GameObjectPtr)
	{
	}
	SeekSteering::~SeekSteering() {}

	Vec3 SeekSteering::Execute(const Vec3& Force, const Vec3& Velocity, const Vec3& TargetPos) {
	...
	///	ArriveSteering行動クラス
	//--------------------------------------------------------------------------------------
	ArriveSteering::ArriveSteering(const std::shared_ptr<GameObject>& GameObjectPtr) :
		SteeringBehavior(GameObjectPtr),
		pImpl(new Impl())
	{
	}
	...
	Vec3 ArriveSteering::Execute(const Vec3& Force, const Vec3& Velocity, const Vec3& TargetPos) {
		auto TransPtr = GetGameObject()->GetComponent<Transform>();
		Vec3 TempForce = Force;
		Vec3 WorkForce;
	...
	PursuitSteering::PursuitSteering(const std::shared_ptr<GameObject>& GameObjectPtr) :
		SteeringBehavior(GameObjectPtr)
	{
	}
	PursuitSteering::~PursuitSteering() {}

	Vec3 PursuitSteering::Execute(const Vec3& Force, const Vec3& Velocity, const Vec3& TargetPos, const Vec3& TargetVelocity, const Vec3& TargetRotation) {
		auto TransPtr = GetGameObject()->GetComponent<Transform>();
		Vec3 TempForce = Force;
		Vec3 WorkForce;
		WorkForce = Steering::Pursuit(Velocity, TransPtr->GetWorldPosition(),
			bsmUtil::normalize(TransPtr->GetRotation()), GetMaxSpeed(),
			TargetVelocity, TargetPos,
			bsmUtil::normalize(TargetRotation)) * GetWeight();
		Steering::AccumulateForce(TempForce, WorkForce, GetMaxForce());
		return TempForce;
	}
	...
	//--------------------------------------------------------------------------------------
	struct WanderSteering::Impl {
		Vec3 m_WanderTarget;	//徘徊目標の回転係数（操舵によって書き換えられる）
	...
	//アクセサ
	const Vec3& WanderSteering::GetWanderTarget() const {
		return pImpl->m_WanderTarget;
	}
	void WanderSteering::SetWanderTarget(const Vec3& target) {
	...
	//--------------------------------------------------------------------------------------
	struct WallAvoidanceSteering::Impl {
		std::vector<PLANE> m_PlaneVec;//回避すべき面の配列
		Impl()
		{
		}
		~Impl() {}
	...
	//--------------------------------------------------------------------------------------
	WallAvoidanceSteering::WallAvoidanceSteering(const std::shared_ptr<GameObject>& GameObjectPtr) :
		SteeringBehavior(GameObjectPtr),
		pImpl(new Impl())
	{
	}
	...
	//アクセサ
	std::vector<PLANE>& WallAvoidanceSteering::GetPlaneVec() const {
		return pImpl->m_PlaneVec;
	}
	void WallAvoidanceSteering::SetPlaneVec(const std::vector<PLANE>& planevec) {
		//必ずクリアする
	...
	void WallAvoidanceSteering::SetPlaneVec(const std::vector<Plane4>& planevec) {
		//必ずクリアする
		pImpl->m_PlaneVec.clear();
		for (auto& v : planevec) {
			PLANE p(v);
	...
	Vec3 WallAvoidanceSteering::Execute(const Vec3& Force, const Vec3& Velocity) {
		auto TransPtr = GetGameObject()->GetComponent<Transform>();
		Vec3 TempForce = Force;
		Vec3 WorkForce(0, 0, 0);
	...
	//--------------------------------------------------------------------------------------
	struct ObstacleAvoidanceSteering::Impl {
		std::vector<SPHERE> m_ObstacleSphereVec;		//回避すべき障害物のSPHERE配列
		float m_RoadWidth;
		float m_RoadHeight;
		Impl() :
			m_RoadWidth(1.0f),
	...
	//--------------------------------------------------------------------------------------
	ObstacleAvoidanceSteering::ObstacleAvoidanceSteering(const std::shared_ptr<GameObject>& GameObjectPtr) :
		SteeringBehavior(GameObjectPtr),
		pImpl(new Impl())
	{
	}
	...
	//アクセサ
	//障害物の配列
	const std::vector<SPHERE>& ObstacleAvoidanceSteering::GetObstacleSphereVec() const {
		return pImpl->m_ObstacleSphereVec;
	}
	void ObstacleAvoidanceSteering::SetObstacleSphereVec(const std::vector<SPHERE>& spherevec) {
	...
	Vec3 ObstacleAvoidanceSteering::Execute(const Vec3& Force, const Vec3& Velocity) {
		auto TransPtr = GetGameObject()->GetComponent<Transform>();
		Vec3 TempForce = Force;
		Vec3 WorkForce(0, 0, 0);
	...
	//--------------------------------------------------------------------------------------
	FollowPathSteering::FollowPathSteering(const std::shared_ptr<GameObject>& GameObjectPtr) :
		SteeringBehavior(GameObjectPtr),
		pImpl(new Impl())
	{
	}
	...
	//アクセサ
	void FollowPathSteering::SetPathList(const std::list<Vec3>& pathlist) {
	...
	Vec3 FollowPathSteering::Execute(const Vec3& Force, const Vec3& Velocity) {
		auto TransPtr = GetGameObject()->GetComponent<Transform>();
		Vec3 TempForce = Force;
		if (IsFinished()) {
	...
	//--------------------------------------------------------------------------------------
	SeparationSteering::SeparationSteering(const std::shared_ptr<GameObject>& GameObjectPtr) :
		SteeringBehavior(GameObjectPtr),
		pImpl(new Impl())
	{
	}
	...
	//アクセサ
	std::shared_ptr<GameObjectGroup> SeparationSteering::GetGameObjectGroup() const {
		auto shptr = pImpl->m_Group.lock();
		if (!shptr) {
			throw BaseException(
				L"グループは無効です",
	...
	void SeparationSteering::SetGameObjectGroup(const std::shared_ptr<GameObjectGroup>& Group) {
		pImpl->m_Group = Group;
	}

	Vec3 SeparationSteering::Execute(const Vec3& Force) {
		Vec3 TempForce = Force;
		Vec3 WorkForce;
